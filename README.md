...
若是 leader 宕机，那么其他节点在可用节点足够的情况下会重新选举出 leader， 然后通过 LeaderCh 获取leader变更通知。除此以外还需要考虑剩余可用节点不足的情况，此时就需要有一个自动检查机制去实时检查并更新集群状态，以免集群状态未能及时更新从而导致数据不一致。若由两个节点组成的集群（hashicrop/raft 两个点亦可选主），在主节点宕机后Follower 节点的状态会进入到Candidate, 但因为目前集群仅剩一个节点，所以也就无法选主。
